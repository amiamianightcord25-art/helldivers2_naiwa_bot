...
- 游戏状态消息必须由用户或群管理员显式订阅。开启群推送、绑定私聊或机器人重启都不会自动订阅星球、DSS、战况等内容。
- 机器人版本更新日志向符合条件的目标按版本发送一次：群须开启推送且至少有一个启用订阅；普通私聊须完成指定用户绑定，可没有游戏订阅。公告配置还可限制场景，不支持频道主动公告。发送 `更新日志` / `changelog` 可主动查看。

私聊主动消息仅向维护者配置的指定用户开放；群消息仅向已经由管理员开启推送的群开放。
QQ 官方消息使用 OpenID，不能把 QQ 号码直接当作 API 收件人，首次使用需要私聊配对：

1. 在 `.env` 设置 `QQ_PUSH_USER=<指定用户QQ号>`，留空则禁用指定用户私聊推送。
2. 维护者运行 `.venv\Scripts\python scripts/pair_push_user.py` 生成一次性绑定码。
3. 使用上述 QQ 账号私信机器人 `绑定推送 <绑定码>`，有效期 1 小时，不要在群里发送。
4. 绑定后在该私聊中按需订阅游戏状态。单纯绑定不会自动创建游戏订阅；绑定目标可以接收一次性版本更新日志。

绑定记录保存平台下发的真实 OpenID，不依赖用户输入的收件人。令牌只供指定账号配对，
它确认的是持有码的私聊会话，QQ 平台本身不提供反查这个 OpenID 对应 QQ 数字号的能力。
更换允许的 QQ 号会立即令旧绑定失效，必须重新配对。
...
- `订阅 战况 60`：每 60 分钟尝试发送简短战况摘要，首次在一个间隔之后；允许 30–1440 分钟。
- 主线：指令出现/结束或任务达成状态变化时推送。
- 防守：进行中的防守事件新增或结束时推送。
- 星球：控制方、防守事件或解放进度跨越 10% 档位时推送。未知字段不会按零处理。
- 新闻 / 补丁：仅新增消息；补丁只接受 Steam 官方来源中已识别的补丁，普通新闻和正文编辑不触发。
- 公告：全球事件新增、结束或撤下时推送。
- DSS：已知驻留星球、战术行动状态、选举轮换、领先目标变化或投票占比跨越 5% 档位时推送；微小票数和物资筹备数字增长不反复触发。
- 战役：新增、结束或撤下、已知控制方变化；不会仅凭战役消失就宣称胜负。
- 区域：当前区域新增、结束、已知控制方或可进入状态变化；人数和每次血量变化不触发。
- 变化订阅先建立基线，不立即补发旧消息。相同状态、API 故障或陈旧缓存均不发送。
- `取消订阅` 取消当前会话全部项目；带主题仅取消对应项目。CLI 不会创建订阅。

只有明确订阅且通过访问检查的目标会收到对应游戏状态；版本更新日志只使用同一批已有主动消息资格目标，不接受命令中的其他收件人参数。
订阅入口、轮询取数前、发送前以及 QQ 发送边界均检查目标；绑定完成前不会推送。
默认每个目标滚动 24 小时最多 5 条，相邻至少 5 分钟；这是本地限额，不代表平台授予了相同额度。
定期检查约每 60 秒一次；不在线或达本地额度时可能延后。电脑关闭、休眠或程序停止期间不会推送。

主动消息通过 QQ 官方 API 发送，不冒用旧 `msg_id` 伪装成被动回复，且只发短文字。
能否实际送达由平台的审核状态、主动消息权限、次数额度及群/用户设置决定。
平台拒绝时会暂停订阅，`订阅列表` 能看到原因，再次订阅可恢复；网络暂时故障使用退避重试。
未获得主动权限的应用仍可正常使用普通查询，本项目不会绕过平台限制。
